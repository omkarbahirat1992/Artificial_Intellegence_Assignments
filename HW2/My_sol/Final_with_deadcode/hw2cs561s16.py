...
Added "query count" check while readingn the "input.txt"
========================================================
'''
...
#        print "not all are grounded theta = %s" %theta
        if theta == None:
#            print "theta is none"
            return False
    for obj in goal.args:
...
#                print "returning False for var:%s" %obj.op
                return False
#    print "returning True for goal:%s" %goal.op
    return True

def FOL_BC_OR(KB, goal, theta):
#    print "\n============in FRESH FOL_BC_OR. goal = %s=======" %goal

    # Need to dump "Ask:" log for the query again iff previous attempt was "False"
    # prev_attempt is said to be False if "ALL PREDICATES" in KB matching the "GOAL PREDICATE" turn out to be FALSE
    # Don't dump "Ask:" log for goal in following cases:
        #1) if all arguments goal are grounded and log "Ask:" is already dumped.
        #2) if atleast one of the argument is not grounded and it cannot be unified with rest of the KB.
...
#            print "$$$$$$$$$$FOL_BC_OR theta1 = %s$$$$$$" %theta1
            ...
            #TODO: think about variable camparison match requirement
...
#works perfect for queries with "&&" in them
def FOL_BC_FIRE_QUERY1(KB, query):
 
    query_obj = make_object(query)
    log = "True"
    try:
        FOL_BC_OR(KB, query_obj, {}).next()
    except:
        log = "False"
    write_log(log)
...
'''
    log = "False"
    for obj in FOL_BC_FIRE_QUERY(fol_kb, query):
       log = "True"
'''   
#    write_log(log)
    
#    print "This is output: %s" % FOL_BC_FIRE_QUERY(fol_kb, query)
