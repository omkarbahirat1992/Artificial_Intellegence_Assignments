...
def write_next_state(mat):
	if simulation:o_file = open("trace_state.txt", "a")
...
#TODO: remove this row_column variable
	...
	if (cur_depth == given_depth):
		return (evaluate(cost_mat, state_mat, given_player))
...
#TODO: player passed over here should be dynamic
		vab[val] = evaluate(cost_mat, state_mat, given_player)
		return vab
...
def make_move(cost_mat, state_mat, player, opponent, algo):
	...
	if (algo == 1):
		...
		print "algo =2"
		key = min_max('root', cost_mat, state_mat, 0, player, opponent, 'MAX')

	elif (algo == 3):
		print "algo =3"
		vab = alpha_beta('root', cost_mat, state_mat, 0, player, opponent, 'MAX', -2000, +2000)
...
def battle_simulation(cost_mat, state_mat):
	print "hi"
	while (1):
		if (make_move(cost_mat, state_mat, algo) == False):
			break
		if (make_move(cost_mat, state_mat,algo2) == False):
			break		
...
def main(argv):
	...
	if (algo == 1):
		GBFS(cost_mat, state_mat, given_player)
		write_next_state(state_mat)

	elif (algo == 2):
	#	print "algo =2"
		key = min_max('root', cost_mat, state_mat, 0, given_player, given_opponent, 'MAX')
		write_traverse_log_file("root", 0, key)

	elif (algo == 3):
	#	print "algo =3"
		vab = alpha_beta('root', cost_mat, state_mat, 0, given_player, given_opponent, 'MAX', -2000, +2000)
		write_ab_traverse_log_file("root", 0, vab[0], vab[1], vab[2])

	if (algo2 > 0):
		print "hi2"
	#	battle_simulation(cost_mat, state_mat)
