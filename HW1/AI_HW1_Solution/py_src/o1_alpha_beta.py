...
def alpha_beta(node, cost_mat, state_mat, cur_depth, player, opponent, flag):
	global row
	global column
	final_state_mat = []
	alpha = -2000
	...
	for i in range(len(state_mat)):
		for j in range(len(state_mat)):
			if (state_mat[i][j] == '*'):
				...
				if (flag == 'MAX'):
					write_ab_traverse_log_file(node, cur_depth, alpha, alpha, beta)
					temp_max = alpha_beta(get_node(i, j), cost_mat, temp_mat, cur_depth + 1, opponent, player, 'MIN')
					if(temp_max < beta):
						beta = temp_max
					write_ab_traverse_log_file(get_node(i, j), cur_depth + 1, temp_max, alpha, beta)
					if (temp_max > alpha):
						alpha = temp_max
						final_state_mat = temp_mat
						row = i
				...
				if (flag == 'MIN'):
					write_ab_traverse_log_file(node, cur_depth, beta, alpha, beta)
					temp_max = alpha_beta(get_node(i, j), cost_mat, temp_mat, cur_depth + 1, opponent, player, 'MAX')
					write_ab_traverse_log_file(get_node(i, j), cur_depth + 1, temp_max, alpha, beta)
					#if (temp_max > alpha):
					#	alpha = temp_max
					if (temp_max < beta):
...
#	read_file(file)
	...
	alpha_beta('root', cost_mat, state_mat, 0, 'X', 'O', 'MAX')
#	print "\ngiven depth = \n" + str(given_depth)
