...
#TODO: player passed over here should be dynamic
	...
	for i in range(len(state_mat)):
		for j in range(len(state_mat)):
			#print "cur_depth = " + str(cur_depth)
			#print "given_depth = " + str(given_depth)
			#print "i = " + str(i) + str(len(state_mat))
			#print "j = " + str(j) + "\n"	
			if (state_mat[i][j] == '*'):
				...
				if (flag == 'MAX'):
					write_traverse_log_file(i, j, cur_depth, max_key)
				...
				if (flag == 'MIN'):
					write_traverse_log_file(i, j, cur_depth, min_key)
...
#TODO: Make "player" input driven
	key = min_max(cost_mat, state_mat, 0, 'X', 'O', 'MAX')
	write_traverse_log_file(1, 1, 0, key)
